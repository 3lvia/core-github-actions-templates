...
- name: Build
  ...
  with:
    ...
    registry:
    # What container registry to use, either `acr` or `ghcr`. If set to `acr`, credentials for Azure Container Registry will default to Elvia values. You can also set these explictly to point to your own ACR. Using ACR requires the permissions `id-token: write` to access the registry using OIDC.  If set to `ghcr`, the action will use the GitHub Container Registry. Using GHCR requires the `packages: write` permission to push to the registry.
    #
    # Required: no
    # Default: 'acr'
