...
| `docker-disable-cache`      | Disable Docker layer caching. When `true`, the build will not use cached layers from the registry. Defaults to `true` on re-runs and manual workflow dispatches, ensuring fresh builds pick up security patches.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | no       | `${{ github.run_attempt > 1 |     | github.event_name == 'workflow_dispatch' }}` |
